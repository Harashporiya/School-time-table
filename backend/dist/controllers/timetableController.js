...
export const upsertTimetable = async (req, res) => {
    try {
        ...
        if (!timePeriod.isBreak && teacherId) {
            ...
            const conflict = await checkTeacherConflict(teacherId, day, timePeriod.startTime, timePeriod.endTime, timePeriodId, existingEntry?.id);
            if (conflict) {
...
export const getAvailableTeachers = async (req, res) => {
    try {
        ...
        const busyTeacherIds = new Set(busyTeachers.map(t => t.teacherId).filter(Boolean));
